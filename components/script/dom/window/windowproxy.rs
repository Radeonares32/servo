...
unsafe extern "C" fn get_own_property_descriptor(
    ...
    if let Some(named_child_navigable) = named_child_navigable(cx, proxy, id) {
        ...
        named_child_navigable.safe_to_jsval(cx, window_proxy_value.handle_mut());
...
unsafe extern "C" fn get(
    ...
    if is_platform_object_same_origin(cx, proxy) {
        ...
        if let Some((window, _attrs)) = window {
            window.safe_to_jsval(cx, return_value);
            return true;
        }
...
/// <https://html.spec.whatwg.org/multipage/nav-history-apis.html#windowproxy-delete>
#[expect(unsafe_code)]
